...
if (isset($_POST["zahtjev"])) {
    ...
    if (isset($_POST["posalji"])) {
        ...
        $upit = "insert into informacije_o_servisu (id_informacije,opis_radnji,cijena_servisa) values ('{$zahtjev}', '{$radnje}', '{$cijena}')";
        $rezultat2 = $baza->updateDB($upit);
        
        $upit3="update zahtjev_za_servis set informacije_o_servisu = '{$zahtjev}' where id_zahtjev = '{$zahtjev}'";
        $rezultat3=$baza->updateDB($upit3);
        
        $upit5="update zahtjev_za_servis set status_id_status = '5' where id_zahtjev = '{$zahtjev}'";
        $rezultat5=$baza->updateDB($upit5);
    }
            ...
            <div>
                ...
                <form novalidate enctype="multipart/form-data" method="post" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                    <select  style="width: 70%; height: 30px;" id="zahtjev" name="zahtjev">
                        <?php
                        $upit = "select t.id_termin, z.id_zahtjev, k.korisnicko_ime, z.naziv as 'naziv_zahtjeva', z.opis, t.naziv as 'naziv_termina', s.opis_statusa from zahtjev_za_servis z join korisnik k on z.korisnik_id_korisnik=k.id_korisnik join termin t on z.termin_id=t.id_termin join status s on z.status_id_status = s.id_status where z.status_id_status = '8'";
                    ...
                    <textarea style="width:70%" type="text" rows="3" id="radnje" placeholder="Radnje na servisu, zamjenjeni dijelovi..." name="radnje"></textarea><br>

                    <input type="text"  id="cijena" placeholder="Cijena servisa" name="cijena"><br>
                    
                    <button type="submit" id="posalji" name="posalji"> Pošalji informacije i završi zahtjev  </button>
